    ...
    function DrawCircle(center, radius, slices){
        ...
        for (var i=0;i<=slices; i++){
            ...
            pos.push((Math.sin(angle)* radius + center.x) * height / width * Depth ,(Math.cos(angle)* radius +center.y));
        }
        return pos;
    }
    ...
    function CreateObstacles(dest, width ,height){
        ...
        gl.clear(gl.COLOR_BUFFER_BIT);
        var Prg = CreateProgram('Obstacles.Vertex', 'Obstacles.Fill');
        // locationの初期化
        gl.useProgram(Prg);
        ...
        var Cposition =DrawCircle({x:0,y:0}, 0.2, 100);
        var vCircle = CreateVbo(Cposition);
        var circleVBOList = [vCircle];
        SetAttribute(circleVBOList, AttLocation, AttStride);
